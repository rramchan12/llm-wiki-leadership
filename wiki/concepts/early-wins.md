---
title: "Early Wins"
category: concept
tags: [transitions, credibility, momentum, leadership, strategy, new-role]
sources: [watkins-first-90-days, kotler-art-of-impossible, heath-made-to-stick, heath-switch, heath-power-of-moments]
created: 2026-04-20
updated: 2026-04-25
---

# Early Wins

**Definition:** Visible, meaningful results secured in the first 30–60 days of a new role (or project) that build personal credibility, create positive momentum, and establish the foundation for larger changes.

## Why It Matters

Credibility is the currency of new leadership. Before people trust your judgment on big changes, they need evidence you understand the situation and can deliver. Early wins provide that evidence. They also create psychological momentum: small successes beget larger successes, and people want to be on a winning team.

## What Makes a Good Early Win

| Characteristic | Why It Matters |
|---|---|
| **Visible** | People need to see it. Private wins don't build credibility. |
| **Meaningful** | Must matter to stakeholders, not just to you. |
| **Quick** | Delivered in weeks, not months. Time pressure is real. |
| **Aligned** | Consistent with the direction you intend to go. Don't win in a way that contradicts your strategy. |
| **Not zero-sum** | Best wins create value; avoid wins that come at others' expense. |

## Common Mistakes

- Pursuing wins that matter to you but not to your boss or team
- Going for a big, risky early bet instead of a sure, visible result
- Winning at the expense of key stakeholders (creating enemies, not allies)
- Mistaking activity for wins — being busy ≠ delivering results

## How Books Treat This

| Book | Stance | Key Insight |
|------|--------|-------------|
| [[watkins-first-90-days]] | Defines | Early wins in the first 30–60 days build the credibility needed for everything that follows. Match your wins to the [[transition-acceleration]] strategy. |
| [[kotler-art-of-impossible]] | Parallels | Daily clear goals serve the same function in the peak-performance context: each small win sustains the motivation stack and keeps the brain's reward system engaged. |
| [[kotler-gnar-country]] | Parallels | [[incremental-progression]] is the early-wins strategy for skill acquisition: each micro-improvement builds confidence and keeps fear manageable. |
| [[heath-decisive]] | Supports | "Ooching" — running small experiments — is a form of early win: low-cost, informative, credibility-building. |
| [[heath-made-to-stick]] | Supports | Wins that aren't communicated don't build momentum. Use [[sticky-communication]] (SUCCESs) to make wins visible, concrete, and memorable. |
| [[heath-switch]] | Supports | "Shrink the change" (Motivate the Elephant) is the early-wins strategy for change initiatives: make the first step so small it's impossible to fail. |
| [[heath-power-of-moments]] | Extends | An early win framed as a [[defining-moments|defining moment]] — with elevation, recognition, and shared celebration — has outsized impact on team momentum. |

## Connections

- Related: [[transition-acceleration]], [[incremental-progression]], [[goal-setting]], [[sticky-communication]], [[rider-elephant-path]], [[defining-moments]]
- Applied in: [[starting-a-new-leadership-role]], [[pursuing-impossible-goals]]
